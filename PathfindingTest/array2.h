...
public:
	Array2();
	Array2(unsigned int width, unsigned int height, T defVal = T());
	Array2(const Array2& rhs);
	~Array2();

	// Resizes the array specified and sets new values to the defaultVal
	void Resize(unsigned int width, unsigned int height, T defVal = T());
...
template< class T >
T& Array2<T>::operator[](const unsigned int index)
{
...
template< class T >
const T& Array2<T>::operator[](const unsigned int index) const
{
	assert(index < (mWidth * mHeight));
	return mData[index];
}
